...
app.post('/api/movies', async (req, res) => {
  try {
    ...
    const data = await client.query(`
      ...
      RETURNING id, name, genre, year, director, country, length, url, user_id as "userId";
      `, [movie.name, movie.genre, movie.year, movie.director, movie.country, movie.length, movie.url, 1]);
    ...
    const data = await client.query(`
      UPDATE movies
        SET name = $1, genre = $2, year = $3,
            director = $4, country = $5, length = $6, url = $7
        WHERE id = $8
        RETURNING id, name, genre, year, director, country, length, url, user_id as "userId";
        `, [movie.name, movie.genre, movie.year, movie.director, movie.country, movie.length, movie.url, req.params.id]);
